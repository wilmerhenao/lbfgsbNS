...
program driver
  ...
  real(dp), parameter    :: factr  = 1.d+1, pgtol  = 0.0d0, &
       tlimit = 1000.0d0, taux = 1.0d-3
  !
  character(len=120)      :: task, csave
  logical                :: lsave(4)
  integer                :: isave(47), xindex, thisiter
  real(dp)               :: f, r, epsilon
  real(dp)               :: dsave(30)
  integer,  allocatable  :: nbd(:), iwa(:)
  real(dp), allocatable  :: x(:), l(:), u(:), g(:), wa(:)
  !
  real(dp)               :: t1, t2, time1, time2, p, z, r1, y
  integer                :: i, j, k, m, n, nfg, startx, startg
...
!          /,5x, ' (f = 0.0 at the optimal solution.)',/) 
     ...
     do while( task(1:2).eq.'FG'.or.task.eq.'NEW_X'.or. &
        ...
        !write (*,*) 'Temporary Value of f = ', f ', iterate :=', isave(30)
...
1844          continue
                 ...
                 ! TEMPWRITE
                 if ((thisiter < isave(30)) .or. (isave(30) .eq. 0)) then
                    write (*,*) 'partial results iteration, f:', isave(30), ",", f, ";", thisiter , ";", dsave(2)
                 !   if (abs(dsave(2) - f) < epsilon) then
                 !      write(*, *) 'I should get out of here'
                 !      task = 'STOP: TOLERANCE OF CONVERGENCE IS < EPSILON'
                 !   end if
                 !   
                 end if
                 thisiter = isave(30)
